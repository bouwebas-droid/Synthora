    ...
    try:
        ...
        for _ in range(6):
            ...
            try:
                ...
                events  = factory.events.PoolCreated.get_logs(from_block=current_block - 1, to_block="latest")
                for ev in events:
    ...
    while True:
        ...
        try:
            ...
            if current <= last_block:
                continue
            factory = w3s.eth.contract(address=AERODROME_FACTORY, abi=FACTORY_ABI)
            events  = factory.events.PoolCreated.get_logs(from_block=last_block + 1, to_block=current)
            if events:
                tasks = []
                for ev in events:
                    t0   = ev["args"]["token0"]
                    t1   = ev["args"]["token1"]
                    pool = ev["args"]["pool"]
                    blk  = ev.get("blockNumber", current)
...
async def cmd_buy(update, context):
    ...
    try:
        factory = w3s.eth.contract(address=AERODROME_FACTORY, abi=FACTORY_ABI)
        events  = factory.events.PoolCreated.get_logs(from_block=w3s.eth.block_number - 100000, to_block="latest")
        pool    = None
        for ev in events:
            t0, t1 = ev["args"]["token0"], ev["args"]["token1"]
