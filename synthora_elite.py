...
# =============================================================
#  WEB3 VERSIE DETECTIE - past zich automatisch aan
# =============================================================
import web3 as _web3_module
_web3_version = tuple(int(x) for x in _web3_module.__version__.split(".")[:2])
WEB3_MAJOR = _web3_version[0]
logger.info("web3 versie: %s (major=%s)", _web3_module.__version__, WEB3_MAJOR)

def make_get_logs_filter(factory_addr, from_block, to_block, topic):
    """Bouw een get_logs filter die werkt met elke web3 versie."""
    # Web3 v7+: integers, HexBytes topics
    # Web3 v6: hex strings of integers, bytes topics
    # Web3 v5: integers, hex string topics
    if WEB3_MAJOR >= 7:
        return {
            "address":   factory_addr,
            "fromBlock": int(from_block) if from_block != "latest" else "latest",
            "toBlock":   int(to_block) if to_block != "latest" else "latest",
            "topics":    [topic],  # HexBytes werkt in v7
        }
    else:
        topic_hex = topic.hex() if hasattr(topic, "hex") else topic
        if str(from_block).startswith("0x"):
            fb = from_block
        elif from_block == "latest":
            fb = "latest"
        else:
            fb = int(from_block)
        if str(to_block).startswith("0x"):
            tb = to_block
...
def parse_pool_log(log):
    """Parseer een PoolCreated log, compatibel met alle web3 versies."""
    try:
        topics = log["topics"]
        # topic[1] = token0, topic[2] = token1 (indexed)
        t0_raw = topics[1]
        t1_raw = topics[2]
        t0 = Web3.to_checksum_address("0x" + (t0_raw.hex() if hasattr(t0_raw, "hex") else t0_raw)[-40:])
        t1 = Web3.to_checksum_address("0x" + (t1_raw.hex() if hasattr(t1_raw, "hex") else t1_raw)[-40:])
        # data: stable(32) + pool(32) + length(32) => pool is bytes 32-64, adres = laatste 20 bytes
        data_raw = log["data"]
        if hasattr(data_raw, "hex"):
            data = data_raw.hex()
        elif isinstance(data_raw, str):
            data = data_raw[2:] if data_raw.startswith("0x") else data_raw
        else:
            data = data_raw.hex()
        # pool adres = bytes 32-64 van data = chars 64-128, adres = laatste 40 chars
...
async def handle_pending_tx(ws3, tx_hash):
    try:
        ...
        for _ in range(6):
            ...
            try:
                topic = w3s.keccak(text="PoolCreated(address,address,bool,address,uint256)")
                log_filter = make_get_logs_filter(AERODROME_FACTORY, current_block - 1, "latest", topic)
                raw_logs = await rpc_retry(aw3.eth.get_logs, log_filter)
                for log in raw_logs:
                    t0, t1, pool_found = parse_pool_log(log)
                    if not t0:
                        continue
                    if new_token.lower() in [t0.lower(), t1.lower()]:
                        pool_addr = pool_found
                        break
                if pool_addr:
                    break
            except Exception:
    ...
    while True:
        ...
        try:
            ...
            if current <= last_block:
                continue
            topic = w3s.keccak(text="PoolCreated(address,address,bool,address,uint256)")
            log_filter = make_get_logs_filter(AERODROME_FACTORY, last_block + 1, current, topic)
            raw_logs = await rpc_retry(aw3.eth.get_logs, log_filter)
            if raw_logs:
                tasks = []
                for log in raw_logs:
...
async def cmd_buy(update, context):
    ...
    try:
        topic = w3s.keccak(text="PoolCreated(address,address,bool,address,uint256)")
        log_filter = make_get_logs_filter(AERODROME_FACTORY, w3s.eth.block_number - 100000, "latest", topic)
        raw_logs = w3s.eth.get_logs(log_filter)
        pool = None
        for log in raw_logs:
            t0, t1, pool_found = parse_pool_log(log)
            if not t0:
