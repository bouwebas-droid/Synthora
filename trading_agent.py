...
# EntryPoint ABI voor het ophalen van de Nonce en de UserOpHash
ENTRY_POINT_ABI = [
...
async def send_user_operation(call_data, to_address, value=0):
    ...
    async with httpx.AsyncClient(timeout=60.0) as client:
        # Stap A: Gas prijzen van de bundler
        ...
        # --- CRUCIALE FIX: ON-CHAIN HASHING VOOR SIMULATIE ---
        def get_current_hash(op):
            op_tuple = (
            ...
            return ep_contract.functions.getUserOpHash(op_tuple).call()

        # Eerste handtekening voor simulatie
        op_hash = get_current_hash(user_op)
        user_op["signature"] = f"0x{architect_signer.sign_message(encode_defunct(primitive=op_hash)).signature.hex()}"

        # Stap B: Sponsoring (Pimlico)
        ...
        # --- STAP C: DEFINITIEVE HANDTEKENING ---
        final_hash = get_current_hash(user_op)
