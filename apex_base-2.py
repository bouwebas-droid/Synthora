...
🪙 Token: `{new_token[:8]}...{new_token[-6:]}`
💱 Pair: {"Stable" if stable else "Volatile"}

**Safety Checks:**
{chr(10).join(safety['checks'])}
{chr(10).join(['⚠️ ' + w for w in safety['warnings']]) if safety['warnings'] else ''}
...
Auto-Snipe: {"🎯 ON" if bot_state['auto_snipe'] else "⏸️ OFF"}
        ...
        if TELEGRAM_ADMIN_ID:
            try:
                bot.send_message(TELEGRAM_ADMIN_ID, notification, parse_mode="Markdown")
            except Exception as e:
        ...
        if TELEGRAM_BROADCAST_CHANNEL:
            try:
                bot.send_message(TELEGRAM_BROADCAST_CHANNEL, notification, parse_mode="Markdown")
            except Exception as e:
                logger.error(f"Failed to send to broadcast channel: {e}")
