#!/usr/bin/env python3
"""
Synthora Elite 123456 - Advanced DeFi Trading Bot
Autonomous sniping on Aerodrome Finance (Base Network)

Version: 123456 (Fixed Telegram Commands + Web3 v7 Compatibility)
"""
...
# ⚠️ Check for both possible env var names
TELEGRAM_TOKEN = os.getenv('TELEGRAM_TOKEN') or os.getenv('TELEGRAM_BOT_TOKEN') or "PASTE_YOUR_TELEGRAM_TOKEN_HERE"
OPENAI_API_KEY = os.getenv('OPENAI_API_KEY') or "PASTE_YOUR_OPENAI_KEY_HERE"
ALCHEMY_API_KEY = os.getenv('ALCHEMY_API_KEY') or "PASTE_YOUR_ALCHEMY_KEY_HERE"
PRIVATE_KEY = os.getenv('PRIVATE_KEY') or os.getenv('ARCHITECT_SESSION') or "PASTE_YOUR_PRIVATE_KEY_HERE"

# Validate critical tokens
if not TELEGRAM_TOKEN or "PASTE_YOUR" in TELEGRAM_TOKEN:
    logger.error("❌ TELEGRAM_TOKEN not configured!")
    raise ValueError("TELEGRAM_TOKEN not configured")

if not ALCHEMY_API_KEY or "PASTE_YOUR" in ALCHEMY_API_KEY:
    logger.error("❌ ALCHEMY_API_KEY not configured!")
    raise ValueError("ALCHEMY_API_KEY not configured")
...
if not private_key or "PASTE_YOUR" in private_key:
    logger.error("❌ PRIVATE_KEY not configured! Set environment variable or hardcode temporarily.")
...
def cmd_status(message):
    ...
    try:
        response = f"""
📊 <b>Synthora Elite 123456 - Status</b>
...
async def scan_new_pools():
    ...
    logger.info("🔍 Synthora Elite 123456 - Pool scanner gestart...")
...
def main():
    """Main entry point"""
    logger.info(f"✅ Synthora Elite 123456 gestart op {config.WALLET_ADDRESS}")
